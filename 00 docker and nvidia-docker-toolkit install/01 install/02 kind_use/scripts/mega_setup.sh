...
for worker in kind-control-plane kind-worker kind-worker2 kind-worker3; do
...
if [ "$SUCCESS" = true ]; then
    ...
    echo "  - Clúster Kind creado con 1 control-plane y 5 workers"
    echo "  - GPUs montadas en control-plane y 3 workers (nvidia-smi disponible)"
    echo "  - NVIDIA device plugin instalado y corriendo"
    echo "  - Etiquetas GPU configuradas en control-plane y 3 workers"
    echo "  - Validación de acceso GPU exitosa en control-plane y 3 workers"
    echo ""
